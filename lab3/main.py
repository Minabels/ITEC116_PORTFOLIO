...
def get_post_then_format_according_to_user(userID: Optional[int] = None):
    ...
    # Loop through posts and match the userID
    for u in posts:
        if u['userId'] == userID:
            # Collect the related comments for each post
            post_comments = []
            for c in comments:
                if c['postId'] == u['id']:  # Match comments to the post
            ...
            # Append the post with its comments
            data["posts"].append({
                "post_title": u["title"],
                "post_body": u["body"],
